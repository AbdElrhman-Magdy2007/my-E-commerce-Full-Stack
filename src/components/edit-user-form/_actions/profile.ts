...
import { getUpdateProfileSchema } from "@/validations/profile";
...
export const updateProfile = async (
  ...
  const result = getUpdateProfileSchema(translations).safeParse(
  ...
  try {
    ...
    return {
      status: 200,
      message: translations.messages.updateProfileSucesss,
    };
  } catch (error) {
    console.error(error);
    return {
